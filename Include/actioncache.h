  ...
  Action::ActionData operation(
    ...
    if (Iter != cache.end()) {
      ...
      if (!cachedAction.match(args)) {
        if (Action::DEBUG)
          std::cerr << "miss " << std::hex << codeId <<
                " " << std::dec << pc << '\n';
        cachedAction.setIds(args);
        cachedAction.action_ = std::move(builder(args));
      }
if (Action::DEBUG)
  std::cerr << "hit " << std::hex << codeId <<
      " " << std::dec << pc << '\n';
      return cachedAction.action_.get();
    }
if (Action::DEBUG)    std::cerr << "new " << std::hex << codeId <<
    " " << std::dec << pc << '\n';
