...
int _PyObject_HasLen(PyObject *o) {
  return (Py_TYPE(o)->tp_as_sequence &&
          Py_TYPE(o)->tp_as_sequence->sq_length) ||
...
int PyObject_CopyData(PyObject *dest, PyObject *src) {
  ...
  if (!PyObject_CheckBuffer(dest) || !PyObject_CheckBuffer(src)) {
    PyErr_SetString(PyExc_TypeError, "both destination and source must be "
                                     "bytes-like objects");
    return -1;
  }
...
PyObject *PyObject_Format(PyObject *obj, PyObject *format_spec) {
  ...
  if (meth == NULL) {
    if (!PyErr_Occurred())
      PyErr_Format(PyExc_TypeError, "Type %.100s doesn't define __format__",
                   Py_TYPE(obj)->tp_name);
    goto done;
  }
  ...
  if (result && !PyUnicode_Check(result)) {
    PyErr_Format(PyExc_TypeError, "__format__ must return a str, not %.200s",
                 Py_TYPE(result)->tp_name);
    Py_DECREF(result);
    result = NULL;
    goto done;
...
static PyObject *binop_type_error(PyObject *v, PyObject *w,
                                  const char *op_name) {
  PyErr_Format(PyExc_TypeError,
...
static PyObject *binary_op(PyObject *v, PyObject *w, const int op_slot,
                           const char *op_name) {
...
static PyObject *ternary_op(PyObject *v, PyObject *w, PyObject *z,
  ...
  if (w->ob_type != v->ob_type && mw != NULL) {
  ...
  if (z == Py_None)
    ...
    PyErr_Format(PyExc_TypeError,
                 ...
                 "'%.100s', '%.100s', '%.100s'",
                 v->ob_type->tp_name, w->ob_type->tp_name, z->ob_type->tp_name);
  return NULL;
}
...
static PyObject *sequence_repeat(ssizeargfunc repeatfunc, PyObject *seq,
                                 PyObject *n) {
  Py_ssize_t count;
  if (PyIndex_Check(n)) {
...
static PyObject *binary_iop1(PyObject *v, PyObject *w, const int iop_slot,
                             const int op_slot) {
...
static PyObject *binary_iop(PyObject *v, PyObject *w, const int iop_slot,
                            const int op_slot, const char *op_name) {
...
INPLACE_BINOP(PyNumber_InMatrixMultiply, nb_inplace_matrix_multiply,
              nb_matrix_multiply, "@=")
...
Py_ssize_t PyNumber_AsSsize_t(PyObject *item, PyObject *err) {
  ...
  if (!err) {
    ...
    */
    ...
    PyErr_Format(err, "cannot fit '%.200s' into an index-sized integer",
                 item->ob_type->tp_name);
  }
...
PyObject *PyNumber_Long(PyObject *o) {
  ...
  if (PyByteArray_Check(o))
    return _PyLong_FromBytes(PyByteArray_AS_STRING(o), PyByteArray_GET_SIZE(o),
                             10);
...
Py_ssize_t PySequence_Length(PyObject *s) { return PySequence_Size(s); }
...
int PySequence_SetSlice(PyObject *s, Py_ssize_t i1, Py_ssize_t i2,
                        PyObject *o) {
  PyMappingMethods *mp;
...
Py_ssize_t PyMapping_Length(PyObject *o) { return PyMapping_Size(o); }
...
static PyObject *callmethod(PyObject *func, const char *format, va_list va,
                            int is_size_t) {
...
PyObject *_PyObject_CallMethodId(PyObject *o, _Py_Identifier *name,
                                 const char *format, ...) {
  va_list va;
  PyObject *func = NULL;
  PyObject *retval = NULL;
...
PyObject *_PyObject_CallMethodIdObjArgs(PyObject *callable,
...
static PyObject *abstract_get_bases(PyObject *cls) {
  ...
  Py_ALLOW_RECURSION bases = _PyObject_GetAttrId(cls, &PyId___bases__);
  Py_END_ALLOW_RECURSION if (bases == NULL) {
